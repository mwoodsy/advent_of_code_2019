...
def getWirePath(wire):
    ...
    for move in wire:
        ...
        for x in range(distance):
            last = path[-1]
            if direction == 'R':
                path.append(tuple([last[0]+1,last[1]]))
            elif direction == 'L':
                path.append(tuple([last[0]-1,last[1]]))
            elif direction == 'U':
                path.append(tuple([last[0],last[1]+1]))
            elif direction == 'D':
                path.append(tuple([last[0],last[1]-1]))
            else:
                print('Unknown Direction: exit script')
                quit()
    return path
