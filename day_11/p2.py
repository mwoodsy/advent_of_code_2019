...
while isinstance(output, int):
    ...
    input = 0
    if position in path:
        p = len(path) - 1 - path[::-1].index(position) 
        input = colors[p]
    ...
    c = opcode.getErrorCode(codes, [input], index, rIndex)
    output = opcode.getErrorCode(codes, [input], index, rIndex)
